...
class LabJack_U3:
    ...
    def __init__(self, addr_CK = 6010, addr_EN = 6009, addr_DA = 6008):
        ...
        DIO pins are assigned as followed:
            ...
            8 -> 15: EIO0 -> EIO7
            16 -> 19: CIO0 -> CIO7
            通常MTT常用配置为：6010、6009、6008，即使用EIO2、EIO1、EIO0作为CK、EN、DA
        """
    ...
    def writeData_dec(self, data_dec, length, mode, eod:bool):
        """
        将给定的十进制数据写入片内SPI
        ...
        eod: End of Data
            0: 写完数据后不会将EN拉高，后续仍然可以继续对片内SPI进行写数据
            1: 写完数据后自动将EN拉高，完成对片内SPI的写操作，SPI将数据电平送出
        """
        if mode == 0:
